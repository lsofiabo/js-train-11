...
function checkValueAndType(mySet, value) {
  if (mySet.has(value)) {
    return `The set has value ${value} of type ${typeof value}`;
  }
  return `The set does not have value ${value}`;
...
function setToArray(mySet) {
  let arr = [...mySet].filter(x => {return typeof x === 'string'});
  arr.sort();
  return arr;
